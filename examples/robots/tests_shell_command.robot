...
Create service yml file as an example
    ...
    ${RAW_CONTENTS}=    get file  ${PROJECT_PATH}/examples/docker_compose.dist
    ${FIXED_CONTENTS}=  prepare contents    ${CURRENT_OS}    ${RAW_CONTENTS}
    create file  ${PROJECT_PATH}/examples/docker-compose.yml   ${FIXED_CONTENTS}
    ...
    should not be equal  ${EMPTY}   ${STDOUT}
    run  cd examples/ && docker-compose down && cd ..
    remove file  ${PROJECT_PATH}/examples/docker-compose.yml
